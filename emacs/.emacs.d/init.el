...
;; When Emacs is launched as a GUI app on macOS (from Finder, the Dock, or
;; Spotlight) it does NOT inherit your shell's environment, so `PATH' is missing
;; entries like Homebrew's `/opt/homebrew/bin'. That makes Emacs unable to find
;; command-line tools such as `rg' (ripgrep), `git', `fd', etc. This package
;; copies `PATH' (and other env vars) from your login shell into Emacs so those
;; tools resolve correctly. It runs early so every package below benefits.
(use-package exec-path-from-shell
  :ensure t
  :straight t
  :if (or (memq window-system '(mac ns x)) (daemonp)) ;; Only needed for GUI/daemon sessions.
  :config
...
(use-package emacs
  ...
  (set-face-attribute 'default nil :family "Berkeley Mono"  :height 100)
...
;;; TRAMP
;; TRAMP (Transparent Remote Access, Multiple Protocols) is built into Emacs and
;; lets you edit files on remote machines as if they were local. Just open a
;; remote path with the usual `C-x C-f', for example:
;;   /ssh:user@host:/path/to/file          ;; Edit a file over SSH.
;;   /sudo::/etc/hosts                      ;; Edit a local file as root.
;;   /ssh:host|sudo:host:/etc/nginx.conf    ;; Hop to a host, then become root there.
;; The settings below favor a snappier, more reliable remote experience.
(use-package tramp
  :ensure nil                                     ;; Built-in, no need to fetch it.
  :defer t                                        ;; Load lazily the first time a remote path is used.
  :custom
  (tramp-default-method "ssh")                    ;; Use SSH by default; fast and reuses your ~/.ssh/config.
  (tramp-verbose 1)                               ;; Keep logging quiet (raise to 6+ only when debugging).
  (remote-file-name-inhibit-cache 60)             ;; Re-check remote file attributes at most once a minute.
  :config
  ;; Don't let version-control backends scan remote directories; this avoids
  ;; noticeable lag when opening files over slow or high-latency connections.
  (setq vc-ignore-dir-regexp
        (format "\\(%s\\)\\|\\(%s\\)"
                vc-ignore-dir-regexp
                tramp-file-name-regexp))
  ;; Reuse the remote login shell's PATH so remote programs are found.
...
(use-package which-key
  ...
  :config
  ;; Friendly names for Evil leader-key prefixes shown by Which-Key.
...
(use-package lsp-mode
  ...
  :hook (;; Replace XXX-mode with concrete major mode (e.g. python-mode)
         ...
         ((js-mode                                      ;; Enable LSP for JavaScript
           ...
           c-mode                                        ;; Enable LSP for C
           c++-mode                                      ;; Enable LSP for C++
           c-ts-mode                                     ;; Enable LSP for C (tree-sitter)
           c++-ts-mode) . lsp-deferred))                 ;; Enable LSP for C++ (tree-sitter)
  :commands lsp
  :custom
...
;;; VTERM
;; A full terminal emulator inside Emacs. This is the right tool for interactive
;; terminal workflows like fzf, shells, REPLs, ssh, htop, and other TUI programs.
(use-package vterm
  :ensure t
  :straight t
  :commands vterm
  ...
  (defun ek/vterm-buffer-name ()
    "Return the project-local vterm buffer name."
  ...
  (defun ek/vterm-toggle ()
    "Toggle a project-local vterm buffer."
    (interactive)
    (let* ((project (project-current nil))
  ...
  (defun ek/vterm-hide ()
    "Hide the selected vterm window."
    (interactive)
    (quit-window nil (selected-window)))
  :config
  (define-key vterm-mode-map (kbd "C-c t") #'ek/vterm-hide)
  (with-eval-after-load 'evil
    ;; Let terminal keybindings such as C-r for fzf shell history pass through.
...
(use-package evil
  ...
  ;; Terminal
  (evil-define-key 'normal 'global (kbd "<leader> t t") 'ek/vterm-toggle)
  ...
  (defun ek/describe-symbol-at-point ()
    "Describe the Emacs Lisp symbol at point."
    (interactive)
  ...
  (defun ek/lsp-describe-and-jump ()
    "Show LSP hover documentation and jump to *lsp-help* buffer."
    (interactive)
    (if (fboundp 'lsp-describe-thing-at-point)
      ...
      (ek/describe-symbol-at-point)))

  (defun ek/describe-thing-at-point ()
    "Show documentation for the symbol or language object at point."
    (interactive)
    (cond
     ...
     ((and (bound-and-true-p lsp-mode)
      ...
      ;; Emacs 31 adds child-frame support in terminals. On Emacs 30, keep the
      ;; buffer-based fallback so documentation remains readable everywhere.
  ...
  (evil-define-key 'normal 'global (kbd "K") #'ek/describe-thing-at-point)
  ...
  ;; Move directly between window splits with C-h/j/k/l, instead of the
  ;; two-step C-w h/j/k/l. These reuse Evil's own window motions, so they
  ;; behave identically to the C-w variants. Note: this shadows the `C-h'
  ;; help prefix while in normal state — use <f1> for help instead.
...
;;; MODUS THEMES
;; The `modus-themes' package provides highly accessible, elegant themes
;; that are built into Emacs. `modus-operandi' is the light variant, designed
;; with a strong emphasis on color contrast for a comfortable reading
;; experience.
(use-package modus-themes
  :ensure t
  :straight t
  :config
  ;; Load the Modus Operandi (light) theme without prompting for confirmation.
  (load-theme 'modus-operandi :no-confirm)

  ;; Use the modus palette so the diff-hl markers pick up theme-aware colors.
  (modus-themes-with-colors
    (custom-set-faces
     ;; Set the color for changes in the diff highlighting to blue.
     `(diff-hl-change ((t (:background unspecified :foreground ,blue))))
     ...
     `(diff-hl-insert ((t (:background unspecified :foreground ,green)))))))
